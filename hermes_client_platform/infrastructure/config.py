...
Umgebungsvariablen (BETTER_HERMES_API_*). Defaults orientieren sich an den
    ...
    def from_extra(cls, extra: dict | None) -> "PlatformSettings":
        ...
        return cls(
            ...
            idle_timeout_seconds=float(
                extra.get("idle_timeout") or env.get("BETTER_HERMES_API_IDLE_TIMEOUT", "1800")
            ),
            http_read_timeout_seconds=float(
                extra.get("http_read_timeout") or env.get("BETTER_HERMES_API_HTTP_TIMEOUT", "300")
            ),
            show_server_tool_markers=bool(
                extra.get("show_server_tool_markers", True)
                if "show_server_tool_markers" in (extra or {})
                else env.get("BETTER_HERMES_API_TOOL_MARKERS", "1") not in ("0", "false", "False")
            ),
        )
